...
export default function Nav() {
  ...
  const link = (href) => ({
    ...
    textDecoration: "none",
    background:
      pathname === href ? "rgba(248,250,252,0.95)" : "rgba(15,23,42,0.16)",
    border:
      pathname === href
    ...
    transition:
      "background-color 140ms, color 140ms, border-color 140ms, box-shadow 140ms, transform 120ms",
  });

  // Link especial para CONTROLE (Admin / Colab) – primeiro e bem destacado
  const adminLink = {
    display: "inline-flex",
    alignItems: "center",
    gap: 8,
    ...
    background:
      pathname === "/dashboard"
        ? "linear-gradient(135deg, #fecaca 0%, #f97373 35%, #ef4444 100%)"
        : "linear-gradient(135deg, rgba(254,202,202,0.95) 0%, rgba(248,113,113,0.9) 45%, rgba(239,68,68,0.9) 100%)",
    border:
      pathname === "/dashboard"
        ? "1px solid rgba(185,28,28,0.95)"
  ...
  // Botão Apoiar (maior e destacado)
  const cta = {
    display: "inline-flex",
    alignItems: "center",
  ...
  return (
    <header style={bar}>
      <div style={wrap}>
        ...
        {/* NAV */}
        <nav style={navWrap}>
          {/* CONTROLE , se tiver permissão */}
          {authReady && canAccessDashboard && (
          ...
          {!user && authReady && (
            <>
              ...
              <Link href="/register" style={link("/register")}>
                {icons.register} Registrar
              </Link>
            </>
          )}
