    ...
    try {
      ...
      try {
        ...
        if (!pixRes.ok) {
          ...
          await showModal(`Pedido registrado, mas houve um erro ao gerar o PIX: ${errMsg}`, "Atenção");
          return;
        }
        ...
        await showModal("Pedido registrado, mas não foi possível gerar o PIX agora. Tente novamente.", "Atenção");
        return;
      }
      ...
      await showModal("Erro inesperado ao finalizar o pedido.", "Erro");
    } finally {
      setSubmitting(false);
    }
  }
            ...
            <>
              <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100">
                ...
                <button
                  type="button"
                  onClick={async () => {
                    if (await showConfirm("Tem certeza que deseja remover todos os itens do carrinho?")) {
                      clearAllItems();
                    }
                  }}
              ...
              <div className="md:hidden divide-y divide-slate-100">
                {items.map((item) => {
                  ...
                  return (
                    <div key={p.id + "-card"} className="flex gap-3 p-4">
                      <img
                        src={cloudinaryUrl(p.imageUrl, { width: 120 }) || "/images/imagem-erro.webp"}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-slate-900 text-base leading-snug">{p.name || p.title}</p>
                        <p className="text-sm text-slate-500 mt-0.5">{p.category || "Sem categoria"}</p>
                        {reservationName && (
                          <p className="text-sm font-semibold text-emerald-700 mt-1">👤 {reservationName}</p>
                        )}
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-sm text-slate-600">{item.quantity}× {formatCurrency(price)}</span>
                          <span className="font-bold text-emerald-600">{formatCurrency(subtotal)}</span>
                        </div>
                      </div>
                      <button
                        type="button"
                        ...
                        className="self-start p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all shrink-0"
                      >
                    ...
                    {items.map((item) => {
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <div>
                                ...
                                {isEventItem && (
                                  <div className="mt-2 space-y-1">
                                    ...
                                    {reservationName && (
                                      <div className="text-sm font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded inline-block mt-1">
                          ...
                          <td className="py-4 px-6 text-base font-medium text-slate-600">
                            {formatCurrency(price)}
                          </td>
                          <td className="py-4 px-6">
                            <span className="inline-flex items-center justify-center min-w-[2.5rem] h-9 px-3 rounded-lg bg-slate-100 text-slate-700 font-bold text-base">
                              {item.quantity}
                            </span>
                          </td>
                          ...
                          <td className="py-4 px-6">
                            <button
                              ...
                              onClick={() => clearEntireProduct(p.id)}
                              aria-label={`Remover ${p.name || p.title} do carrinho`}
                              className="text-red-500 hover:text-red-700 hover:bg-red-50 p-2.5 rounded-lg transition-all"
                            >
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                              </svg>
              ...
              <div className="bg-slate-50 p-6 md:p-8 border-t border-slate-100">
                <div className="grid md:grid-cols-2 gap-8 items-start">
                  <div className="space-y-4">
                    <div>
                      <label htmlFor="pickup-name" className="block text-base font-bold text-slate-700 mb-2">
                        Nome de quem vai retirar <span className="text-red-500">*</span>
                      </label>
                      <input
                        id="pickup-name"
                        className="w-full px-4 py-3 rounded-xl border border-slate-200 text-base focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
                    ...
                    <div>
                      <label htmlFor="pickup-whatsapp" className="block text-base font-bold text-slate-700 mb-2">
